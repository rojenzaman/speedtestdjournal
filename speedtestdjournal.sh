...
cat <<EOF
<!DOCTYPE html> <html> <head> <title>Speed Test Logs</title> <meta http-equiv="content-type" content="text/html;charset=utf-8" /> <meta name="generator" content="speedtestjournal.sh" /> <meta name="viewport" content="width=device-width, initial-scale=1" /> <style> div.gallery { margin: 5px; border: 1px solid; float: left; width: 320px; } div.gallery:hover { border: 1px solid } div.gallery img { width: 100%; height: auto; } div.desc { padding: 15px; text-align: center; } </style> </head> <body> <h1>Speed Test Journal (image logs)</h1> <p>generated by <a href="https://github.com/rojenzaman/speedtestjournal">speedtestjournal.sh</a></p> <hr/> $(image_div) </body></html>
EOF
exit 0
}
