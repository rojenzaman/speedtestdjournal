...
cat <<EOF
<!DOCTYPE html> <html> <head> <title>Speedtest CLI Logs</title> <meta http-equiv="content-type" content="text/html;charset=utf-8" /> <meta name="generator" content="speedtestdjournal.sh" /> <meta name="viewport" content="width=device-width, initial-scale=1" /> <style> div.gallery { margin: 5px; border: 1px solid; float: left; width: 320px; } div.gallery:hover { border: 1px solid } div.gallery img { width: 100%; height: auto; } div.desc { padding: 15px; text-align: center; } </style> </head> <body> <h1>Speedtest CLI Journal (image logs)</h1> <p>generated by <a href="https://github.com/rojenzaman/speedtestdjournal">speedtestdjournal.sh</a></p> <hr/> $(image_div) </body></html>
EOF
exit 0
}
...
_isp_ping=$( (cat $journal_path/json/*.json | jq -r '.client.isp, .ping') | paste - - )
_down_up=$( (cat $journal_path/json/*.json | jq -r '.download, .upload' | xargs numfmt --to iec)  | paste - - )
_date=$( cat $journal_path/json/*.json | jq -r '.timestamp' | date '+%d/%m/%Y %T' -f - )
paste <(cat <<<$_isp_ping) <(cat <<<$_down_up) <(cat <<<$_date)
exit 0
}
